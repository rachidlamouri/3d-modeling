...
import {
  DimensionScript,
  parseExpression,
} from '../expressionParser/parseExpression';
import {
  VariableEquation,
...
export type PartialDimensionDefinitions<
...
export const parsePartialDimensions = <DimensionNames extends VariableLiterals>(
  ...
  _(entries<DimensionNames[number], DimensionDefinition>(definitions))
    ...
    .tap((equations) => {
      ...
      equations.forEach((equation) => {
        if (equation.hasDuplicateVariables()) {
          const duplicateVariableNames = equation
            .getDuplicateVariableNames()
            .join(', ');
          errors.push(
            new Error(
              `Definition \`${equation.toInputString()}\` has duplicate variable(s) "${duplicateVariableNames}"`,
            ),
          );
        }
        ...
        if (equation.hasErrorExpression()) {
          errors.push(
            Error(
              `Definition \`${equation.toInputString()}\` has a parse error \`${equation.toString()}\``,
            ),
          );
        }
      });
    ...
    .map((variableEquation) =>
      isolateVariables<DimensionNames>(variableEquation),
    )
    .reduce((variableEquationSystems, variableEquationSystem) => {
      entries<DimensionNames[number], VariableEquation<DimensionNames>>(
        variableEquationSystem,
      ).forEach(([variableName, variableEquation]) => {
        const equations =
          variableEquationSystems[variableName] ??
          ([] as VariableEquation<DimensionNames>[]);
