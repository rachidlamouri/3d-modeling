...
import(`../../${src}`).then((models) => {
  ...
  modelNames.forEach((modelName) => {
    const childProcess = exec(`npx jscad scripts/loadModel.js --filepath ${src} --model ${modelName} -o ${src.replace(/^src/, 'build/src').replace(/\.ts$/, `.${modelName}.stl`)}"`, {});
